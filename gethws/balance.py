...
class Websocket(WebsocketConnectionView):
    ...
    def printBalance(self,pid):
        ...
        while self.connected:
            yield from asyncio.sleep(pool_interval)
            a = yield from self.get_balance()
            if a != b:
                formatted_balance = web3.fromWei(a,'ether')
                yield from self.ws.send(json.dumps({'eth_balance': a,'formatted_balance': '{:,.4f}'.format(formatted_balance) },cls=DecimalEncoder))
            b = yield from self.get_balance()

    @asyncio.coroutine
    def on_open(self):
        if self.request.matchdict['address'] and web3.isAddress(self.request.matchdict['address']):
